...
clients.get(
  ...
  async (c) => {
    ...
    if (q) {
      ...
      filters.push(
        or(ilike(schema.clients.name, like), ilike(schema.clients.slug, like)),
      );
    }
    if (cursor) {
      ...
      filters.push(
        keysetFilter(schema.clients.createdAt, schema.clients.id, parsed),
      );
    }
    const rows = await db
      .select()
...
clients.patch(
  ...
  async (c) => {
    ...
    const [row] = await db
      .update(schema.sites)
      .set(input)
      .where(and(eq(schema.sites.id, siteId), eq(schema.sites.clientId, id)))
      .returning();
...
clients.post(
  ...
  async (c) => {
    ...
    if (input.siteId) {
      ...
      if (site.clientId !== id) {
        return c.json({ error: "Site does not belong to this client" }, 400);
      }
    }
    if (input.isPrimary) {
      await db
