...
function parseFilter(r: BerReader): FilterNode {
	...
	switch (tag) {
		...
		case 0xa8: {
			...
			if (tag === 0xa5)
				return { type: "gte", attribute, value: attrValue };
			if (tag === 0xa6)
				return { type: "lte", attribute, value: attrValue };
		...
		case 0xa4: {
			...
			while (subs.remaining > 0) {
				...
				if (part.tag === 0x80) initial = readString(part.value);
					...
					throw new BerError(
						`bad substring choice 0x${part.tag.toString(16)}`,
					);
			}
...
export function decodeMessage(
	...
	if (opTag === 0x60) {
		...
		if (auth.tag !== 0x80)
			throw new BerError("only simple bind is supported");
		message = {
			kind: "bindRequest",
			messageId,
			version,
...
export function encodeFilter(node: FilterNode): Uint8Array {
	switch (node.type) {
		...
		case "substrings": {
			...
			return tlv(
				0xa4,
				concat(octet(node.attribute), seq(concat(...parts))),
			);
		}
	}
}
