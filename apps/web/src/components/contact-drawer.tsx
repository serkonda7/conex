...
function toValues(
	...
	return {
		...
		title: c?.title ?? "",
		clientId:
			contactClientId(c ?? ({} as Contact)) || defaultClientId || "",
...
export function ContactForm(props: {
	...
	>({});
	const [syncedKey, setSyncedKey] = createSignal<string | undefined>(
		undefined,
	);
	...
	return (
		<form class="form" onSubmit={submit} novalidate>
			...
			<div class="grid-2">
				<Field label="First name" error={errors().first_name}>
					<input
						...
						value={v().first_name}
						onInput={(e) =>
							set("first_name", e.currentTarget.value)
						}
						placeholder="Ada"
					/>
				</Field>
			...
			<Field
				label="Site"
				hint={sitesQuery.isLoading ? "Loading sites…" : "Optional."}
			>
				<select
					value={v().siteId}
...
export function ContactDrawer(props: {
	...
	const assignMut = createMutation(() => ({
		mutationFn: () => {
			const current = editing();
			if (!current)
				throw new Error("Save the person before moving them.");
				...
				<div class="form">
					...
					<Field label="Target site">
						<select
							value={moveSiteId()}
							onChange={(e) =>
								setMoveSiteId(e.currentTarget.value)
							}
