...
export function ClientForm(props: {
  ...
  const [values, setValues] = createSignal<ClientFormValues>(toValues(props.initial));
  const [errors, setErrors] = createSignal<Record<string, string | undefined>>({});
  const [syncedId, setSyncedId] = createSignal<string | null | undefined>(undefined);
    ...
    <form class="form" onSubmit={submit} novalidate>
      ...
      <Field label="Name" error={errors()["name"]}>
        <input
          ...
          onInput={(e) => {
            ...
            if (!v().slug) set("slug", slugify(e.currentTarget.value));
          }}
      ...
      <div class="form-actions">
        <button type="submit" class="btn btn-primary" disabled={props.saving}>
          {props.saving ? "Saving…" : (props.submitLabel ?? "Save client")}
        </button>
      </div>
    </form>
  );
