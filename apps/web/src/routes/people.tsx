...
const PeoplePage: Component = () => {
  ...
  const sitesQuery = createQuery(() => ({
    ...
    queryFn: () => (clientId() ? fetchSites(clientId()) : Promise.resolve([])),
  }));
  ...
  const askDelete = (c: Contact) => {
    if (window.confirm(`Delete ${contactName(c)}? This cannot be undone.`)) {
  ...
  return (
    <main class="page">
      <div class="page-header">
        <h1>
          People{" "}
          <span class="count">{contactsQuery.data ? `(${contactsQuery.data.length})` : ""}</span>
        </h1>
        <button
          type="button"
      ...
      <div class="toolbar">
        ...
        />
        <select value={clientId()} onChange={(e) => setClientId(e.currentTarget.value)}>
        ...
        <select
          ...
          disabled={!clientId()}
          title={clientId() ? "Filter by site" : "Pick a client first"}
        >
