...
import { IconPencil, IconPlus, IconTrash } from "@tabler/icons-solidjs";
import {
	createMutation,
	createQuery,
	useQueryClient,
...
const ClientsPage: Component = () => {
	...
	const askDelete = (c: Client) => {
		if (
			window.confirm(`Delete client "${c.name}"? This cannot be undone.`)
		) {
	...
	const columns: Col[] = [
		...
		{ key: "slug", header: "Slug" },
		{
			...
			render: (row) => (
				<StatusBadge
					status={String((row as unknown as Client).status)}
				/>
			),
		},
