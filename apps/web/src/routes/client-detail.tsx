...
import { EmptyState, ErrorState, Field, LoadingState } from "../components/ui-helpers.js";
...
const ClientDetailPage: Component = () => {
  ...
  const [siteFieldError, setSiteFieldError] = createSignal<string | undefined>(undefined);
        ...
        {(client) => (
          <>
            <div class="page-header">
              <h1>
                {client().name} <StatusBadge status={String(client().status)} />
              </h1>
              <button
                type="button"
            ...
            <div class="grid-2">
              ...
              <section class="card">
                ...
                <Show when={sitesQuery.isSuccess && sites().length === 0}>
                  <p class="muted">No sites yet — add the first one below.</p>
                </Show>
                ...
                <form class="inline-form" onSubmit={submitSite} novalidate>
                  <Field label="New site name" error={siteFieldError()}>
                    <input
                      class="input"
                      value={siteName()}
                  ...
                  <Field label="City">
                    <input
                      class="input"
                      value={siteCity()}
                      onInput={(e) => setSiteCity(e.currentTarget.value)}
                      placeholder="Berlin"
                    />
                  </Field>
                  <label class="check-row">
                    <input
                      type="checkbox"
                      checked={sitePrimary()}
                      onChange={(e) => setSitePrimary(e.currentTarget.checked)}
                    />
                    <span>Primary</span>
                  </label>
                  <button
                  ...
                  >
                    <IconPlus size={15} /> {siteMut.isPending ? "Adding…" : "Add site"}
                  </button>
                </form>
                <Show when={siteError()}>
            ...
            <section class="card">
              ...
              <Show when={contactsQuery.isSuccess && contacts().length === 0}>
                <EmptyState
              ...
              <Show when={contacts().length > 0}>
                <Table columns={contactColumns} rows={contacts()} />
              </Show>
            </section>
          </>
        )}
