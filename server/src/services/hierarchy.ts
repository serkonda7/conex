...
export function buildParentMap(rows: LocationNode[]): Map<string, string | null> {
...
/** Builds `parent id -> child ids` for subtree walks. */
export function buildChildrenMap(rows: LocationNode[]): Map<string, string[]> {
	const children = new Map<string, string[]>()
	for (const row of rows) {
		const key = row.parent_id ?? '\0ROOT'
		const list = children.get(key)
		if (list) {
			list.push(row.id)
		} else {
			children.set(key, [row.id])
		}
	}
	return children
}
 ...
 */
export function maxDescendantOffset(nodeId: string, children: Map<string, string[]>): number {
	let maxOffset = 0
	const stack: Array<{ id: string; offset: number }> = [{ id: nodeId, offset: 0 }]
